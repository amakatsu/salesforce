...
<div style='background: linear-gradient(90deg, #f56565 0%, #ed8936 100%);
    ...
    <h3 style='color: white; margin: 0;'>💡 画面項目とドメインの突合チェック</h3>
    <p style='color: #fff5f5; margin: 0.5rem 0 0 0; font-size: 0.9rem;'>
        画面項目定義とドメイン定義を突合して整合性を確認
    </p>
</div>
...
with st.expander("💡 使い方を見る", expanded=False):
    ...
    - **ドメイン定義一覧**: ドメイン名、データ型、最大文字数、コード値など
    ...
    - **完全一致**: ドメイン情報を自動取得
    - **類似一致**: 類似ドメインの情報を表示
    - **不一致（提案必要）**: 桁数制約や編集形式があり、ドメイン提案が必要
    - **不一致（チェック不要）**: 桁数・編集形式なし、ドメイン不要
    """)
