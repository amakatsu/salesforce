...
LLM_SYSTEM = (
    ...
    "\n"
    "# 重要: あなたに渡される画面項目名は、既にローカル完全一致判定を通過しています\n"
    "# つまり、完全一致するものは既に除外されており、あなたが判定すべきは以下の2つのみです\n"
    "\n"
    "# 判定基準（厳密に適用）\n"
    "- **一部一致**: 画面項目名を複数の語に分解でき、その一部（1つ以上）が単語帳にある場合\n"
    "  例1: 「顧客担当者名」で「顧客」と「名」が候補にあり、「担当者」は未登録 → 一部一致\n"
    "  例2: 「回収稟議番号」で「回収」と「稟議番号」が別々の候補としてある → 一部一致\n"
    "  例3: 「銀行取引一覧」で「銀行」「取引」「一覧」が候補にある → 一部一致\n"
    "  重要: 候補が1つでもあれば基本的に一部一致です\n"
    "- **一致なし**: 画面項目名のどの語も単語帳に存在しない、または候補が全て不適切な場合のみ\n"
    "  例: 「稟議承認日」で「稟議」「承認」「日」全てが候補にない → 一致なし\n"
    "\n"
    "# 注意: 完全一致と判定してはいけません（完全一致は既にローカルで処理済み）\n"
    "\n"
    "# ネーミングルール（厳格に遵守）\n"
    "\n"
    "## 基本原則\n"
    ...
    "2. 各語が候補リストにあるか確認\n"
    "3. 一致状況を判定:\n"
    "   - 一部一致: 分解した語の1つ以上が候補にある場合（これが基本）\n"
    "   - 一致なし: どの語も候補にない場合のみ\n"
    "   - 注意: 完全一致と判定しないこと（既にローカルで処理済み）\n"
...
フィールド定義:
- match_type: 「一部一致」または「一致なし」のみ（完全一致は禁止）
- matched_term: 常に null（完全一致用フィールドは使用しない）
- matched_terms: 一部一致時の複数語リスト（例: ["顧客", "名"]）、一致なしの場合は null
- proposed_name: 必須、lowerCamelCase、8-15文字推奨
- coverage_ratio: 0.0-1.0、一致なし=null または 0.0
