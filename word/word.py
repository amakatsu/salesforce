...
■ この版での変更点（HTTPS/証明書対応・PyInstaller onefile 対応）
  - Requests が参照する CA バンドルを、実行形態に応じて自動設定
    - PyInstaller onefile 実行時: 展開先 (sys._MEIPASS) の cacert.pem を使用
    - 通常実行時        : certifi の CA バンドルを使用
  - PyInstaller で cacert.pem を同梱する想定

■ ビルド例（Windows / cmd.exe）
  py -m pip install pyinstaller pandas openpyxl python-dotenv requests certifi
  py -c "import certifi; print(certifi.where())"
  REM 上で表示されたパスを <CACERT_PATH> に代入
  py -m PyInstaller --onefile --name TermMatcher ^
    --add-data "<CACERT_PATH>;." ^
    excel_term_matcher.py

■ 実行時の確認ポイント
  - 環境変数 REQUESTS_CA_BUNDLE が _MEIPASS\cacert.pem（onefile時）または certifi のパス（開発実行時）を指していること
  - .env などで VERIFY_SSL=true を推奨
"""
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    # --- シート/列（必要に応じて引数で上書き）
    "SCREEN_SHEET": os.getenv("SCREEN_SHEET", "*画面*,*項目*,*定義*,*"),
    "VOCAB_SHEET": os.getenv("VOCAB_SHEET", "*単語*,*用語*,*"),
...
def main() -> None:
    # === 追加: Requests の CA バンドルを実行形態に応じて設定 ===
    def _bundle_ca_path() -> str:
        # PyInstaller onefile 実行時は同梱物が一時展開され、sys._MEIPASS に配置される
        if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
            return os.path.join(sys._MEIPASS, "cacert.pem")
        # 開発実行時は certifi のバンドルを使う
        import certifi
        return certifi.where()
    os.environ["REQUESTS_CA_BUNDLE"] = _bundle_ca_path()
