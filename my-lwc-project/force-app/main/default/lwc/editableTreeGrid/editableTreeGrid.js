...
export default class EditableTreeGrid extends LightningElement {
    ...
    tree.forEach((node) => {
      ...
      try {
        ...
        const getCellClass = (fieldName) => {
          ...
          const changeClass = this.isFieldChanged(
            node,
            originalNode,
            fieldName,
            highlight,
  ...
  updateNodeInTree(tree, id, field, value) {
    ...
    for (const node of tree) {
      ...
      if (
        node.children &&
        this.updateNodeInTree(node.children, id, field, value)
      ) {
        return true;
      }
    }
