...
export const teamsSearchTool = createTool({
  id: 'teams.search',
  description:
    'Microsoft Teamsで指定されたキーワードを検索し、関連するメッセージやファイルを取得します。チーム内のコミュニケーションや共有ファイルを検索する際に使用します。',
  inputSchema: z.object({
    baseUrl: z.string().url().describe('Teams WebアプリのベースURL'),
  ...
  execute: async ({ context }): Promise<{
    ...
    const result = await invokeMcp('teams.search' as const, {
      baseUrl,
      keyword,
      max,
      timeoutSeconds,
