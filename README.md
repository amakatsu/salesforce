...
> ※ いま、一部ケースで**遷移の本数が減る（漏れる）**報告があったため、**漏れ防止のルール**を強化しました。下記の **RECALL_FAILSAFE / PRE-FLIGHT** を必ず適用してください。
...
* **別画面の判定の補助**：`別／同画面` が空欄や`-`でも、`遷移先（別画面の場合）` が**埋まっている**なら **別画面として扱う**のが安全です（落ち漏れ防止）。
...
* **検収のコツ**：
  ...
  * **行数チェック**：貼り付けた表の**対象行数**と、出力の`--> メインID`本数が**一致**しているか（**他→メイン**は特に）。
  * **モーダル表現**：`種類=モーダル` の行は**ボックス色#FF7F50**・**破線**になっているか。
  * **重複の定義**：**同一（遷移元ID, 遷移先ID, ボタン名）**のみを重複とみなし、**ボタン名が異なる**遷移は**別物**として残す。
...
- **state定義の網羅**：**定義対象ノード集合 S＝{メインID} ∪ OUT ∪ IN の全ID**を**必ず state 定義**する（**他→メインのみ登場する画面も必ず定義**）。
- **重複・統合の扱い（改訂）**：**同一の (遷移元ID, 遷移先ID)** は **矢印1本に統合**し、**ラベルはボタン名を合体**する（順序は**入力順**、重複除去）。**方向が逆**（ID→ID vs ID←ID）は統合しない。
- **件数コメントの出力**：ヘッダー直下に **コメント**で件数を出す（例：`' IN_rows=12, OUT_rows=18, IN_edges=12, OUT_edges=18`）。コメントは**'（シングルクォート）**で始める。

## STATE_BUILD_RULES（INのみ登場する画面も必ず定義）
**生成手順**
1. 画面一覧から **`画面ID → メニュー区分・分割グループ`** の辞書を作る。
2. **S＝{メインID} ∪ OUT ∪ IN** を算出。
3. **S の各ID**について、**必ず state を定義**し、**辞書のグループ**に配置する（モーダルでも呼び出し元に合わせない）。
4. **edge** の出力は、**対象表の該当行のみ**（メイン→他／他→メイン）を矢印化する。
5. **IDが引けない場合**は、画面一覧から**和名で突合**。一意に決まらない／見つからない場合は、
   - `不足情報: ボタン一覧（他→メイン）○行目の画面ID（遷移元）` のように**列名と行番号つきで質問**しつつ、**暫定ノード**を **保持** して矢印も出す（下記RECALL_FAILSAFE）。

## EDGE_MERGE_RULES（同一ペアのラベル合体）
**目的**：同じ画面から同じ画面へ複数のボタンで遷移する場合、**線を1本にまとめ**、**ラベルを合体**して可読性を上げる。

**ルール**
1. **統合キー**：`(遷移元ID, 遷移先ID, 線スタイル)` でグルーピング。
   - **線スタイル**＝デフォルト実線／**遷移先がモーダル**の場合は**破線**（既存ルール）。スタイルが異なるもの同士は**統合しない**。
2. **ラベル合体**：同一グループの**ボタン名**を**入力順**で連結。
   - 連結記号は **` / `**、または**改行**（`\\n` もしくは `%n()`）。
   - **推奨**：4件までは ` / `、5件以上は **改行**で縦積み。
3. **例**：
```plantuml
RgV0201 --> RgV1001 : 開く / 継続 / 条件変更 / 増減額 / 極度超過
' または
RgV0201 --> RgV1001 : 開く%n継続%n条件変更%n増減額%n極度超過
```
4. **リンクノートの活用（任意）**：ラベルが長くなる場合、**note on link** を使ってボタン一覧を別記。
```plantuml
RgV0201 --> RgV1001 : 申請
note right on link
  他のボタン: 継続 / 条件変更 / 増減額 / 極度超過
end note
```

> 補足：改行は従来の `\\n` に加え、関数 **`%n()` / `%newline()`** も利用できます（描画上の改行）。

## RECALL_FAILSAFE（漏れ防止の出力保全）
- **暫定ノードの作法**：ID未特定でも**出力を保全**するため、`UNK_001`のような**一時ID**で state を定義し、**表示名**には分かる範囲の名前を入れる。
  ```plantuml
  state UNK_001 as "（未解決）直近計数照会?"
  note top of UNK_001
    不足情報: ボタン一覧（他→メイン）12行目の画面ID（遷移元）
  end note
  UNK_001 --> RgV0201 : 過去禀査
  ```
- **コメントで件数を宣言**：ヘッダー直下に、**入力行数と出力矢印本数**をコメントで記す（例：`' IN_rows=12, IN_edges=12`）。
- **許容差**：`行数 - 矢印本数 = 0` を原則とし、**差分が1以上**なら**不足情報の質問**と**暫定ノード**を必須化。（メインに紐づかない画面を定義しない）
**関連判定ロジック（生成アルゴリズム）**
...
### ✅ 良い例（他→メイン“のみ”登場する画面の定義）
```plantuml
state 稟議グループ {
  state RgV050201 as "利率変更禀議書・査定書（計数情報） 直近計数照会"
  RgV050201 : RgV050201
}
RgV050201 --> RgV0201 : 過去禀査
```
※ `RgV050201` は **他→メイン表にしか登場しなくても state を定義**し、**正しいグループ**（画面一覧のメニュー区分）に配置する。

### ✅ 良い例（同一ペアの複数ボタンを合体）
```plantuml
' RgV0201 から RgV1001 へ 5ボタンで遷移するケース
RgV0201 --> RgV1001 : 開く / 継続 / 条件変更 / 増減額 / 極度超過
```
...
RgV0201 --> RgV1001 : 開く
RgV0201 --> RgV1001 : 継続
RgV0201 --> RgV1001 : 条件変更
RgV0201 --> RgV1001 : 増減額
RgV0201 --> RgV1001 : 極度超過
```
※ **同一(遷移元ID, 遷移先ID)** は **1本に統合**し、**ラベル合体**する。
...
**入力**：画面一覧に `RgV1901 格付禀議書・査定書` があるが、ボタン一覧（メイン→他／他→メイン）に **該当行なし**。 **NG出力**（定義してしまっている）：
...
2. **画面ID**：**`state 画面ID as "画面名"` に統一**。箱内にIDも表示したい場合は **別行**で `画面ID : 画面ID` を追加。箱内にIDも表示したい場合は **別行**で `画面ID : 画面ID` を追加。
...
| メニュー区分 | 画面ID | 物理名 | 画面名 | 部品 | 種類（親、子、モーダル） | ボタン | ボタン種類 | 別／同画面 | 遷移先（別画面の場合） | | 一覧 | RgV0201 | RinsaLst | 禀議・査定一覧 | 禀議・査定一覧 | 親 | 開く | その他ボタン群（右上） | 別画面 | 融資禀議書・査定書 |

### ボタン一覧（他→メイン：例）

| メニュー区分 | 画面ID | 物理名 | 画面名 | 部品 | 種類（親、子、モーダル） | ボタン | ボタン種類 | 別／同画面 | 遷移先（別画面の場合） | | 稟議 | RgV050101 | YushiRinsashoLastCnt | 融資禀議書・査定書（計数情報）　直近計数照会 | 融資禀議書・査定書（計数情報） 直近計数照会 | 親 | 過去禀査 | その他ボタン群（右上） | 別画面 | 禀議・査定一覧 |

## PRE-FLIGHT（自己検査)

* [ ] メインごとに**前後1段階**のみ（メイン→他／他→メイン）
* [ ] **S＝{メインID} ∪ OUT ∪ IN の全IDが state 定義**されている（**INのみ登場**の画面も含む）
* [ ] **グループ不一致=0**（各 state の配置が **画面一覧のメニュー区分・分割グループ** と一致）
* [ ] **他→メインの対象行数**と**`(遷移元ID, メインID)` の**重複を除いた**組数が一致
* [ ] **メイン→他の対象行数**と**`(メインID, 遷移先ID, 線スタイル)` の**重複を除いた**組数が一致（**モーダル=破線**は別グループ）
* [ ] モーダルは **#FF7F50 + 破線**（グループは自分のメニュー区分）
* [ ] **すべての遷移参照がID（=画面ID）**（表示名参照なし）
* [ ] **識別子に禁止文字**（「・」や括弧、空白など）が**含まれていない**
* [ ] **孤立state=0**（メインと直結しないノードが定義されていない）
* [ ] **差分=0**（`IN_rows-unique_pairs_IN=0`、`OUT_rows-unique_pairs_OUT=0`）。差分がある場合：**不足情報の質問**＋**暫定ノード**で**出力保全**。
* [ ] **重複線なし**（同一ペアで線が複数本になっていない）
